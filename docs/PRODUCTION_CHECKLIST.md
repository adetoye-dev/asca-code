...
| Encryption at rest | **[partial]** | Secrets still sit in a `0600` SQLite file, so **this is not yet encrypted** — the remaining work is named below. Stage one has landed: a `SecretStore` seam with a `KeychainStore` behind it, `engine_resolve_key` now keychain-first, and an **additive** migration that copies a value into the keychain and deliberately **never deletes** the original. No crypto is involved anywhere, because the OS keychain *is* the cipher — which is why this fits in a signed shell and the stdlib-only, frozen Python engine needs no change (its `get_secret` is already environment-first). `keyring = "3"`; v4 exists and Dependabot will offer it. **What the first run of the opt-in real-keychain test proved, and why the delete is not written yet:** the write *succeeded* and a fresh entry could not read it back — `the keychain accepted the write but could not read it back: No matching entry found in secure storage`. That is an unsigned test binary on macOS, refused where it matters. The store therefore verifies every write with a fresh read and reports failure rather than success, the resolver treats a failed copy as best-effort and falls back, and nothing was left behind in the Keychain (checked). Deleting on that evidence would lose a key while looking successful, so the delete waits for a **signed** build to demonstrate the round trip — a release-time check. **Left in this stage:** (a) the delete, gated as above; (b) the frontend write path — `appStore.secrets.*` still writes to SQLite, so new keys never reach the keychain; (c) an OS keychain cannot be enumerated, so "which providers are connected" must become a per-provider lookup rather than a list, which changes `listSecretNames`' callers; (d) `ACSA_SECRET_*` injection into the engine child's environment, needed only once (a) starts removing rows. |
