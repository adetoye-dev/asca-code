...
| Approval affordance | **[partial]** | The live experience this came from: a run sat blocked for roughly six minutes and the only way to find out why was to read the accessibility tree. Two of those causes are fixed. The card is pinned above the composer (the "below the fold" note here predated it), and it now carries `role="alertdialog"` with a label and takes focus when it appears, once per card. `waitingForUser` also reaches the **status bar** — always on screen, as a polite live region — where before it existed only inside the chat, so a blocked run looked like a stopped one to anyone watching the editor. Found while testing that: the composer's autofocus ran after the card's and stole focus back, which is now guarded. Still open: the chat toggle in the titlebar carries no badge, so a user who has the panel closed sees the status bar but not *where* to answer, and the approval and question cards still share one slot. |
